...
from HarleyRobot.modules.sql_extended.night_mode_sql import add_nightmode, rmnightmode, get_all_chat_id, is_nightmode_indb
    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "❗️**12:00 Am Night Mode Activated**❗️ \n_Group Is Closing Till 6 Am_. **Powered By Harley**"
            )
            await tbot(
...
async def job_open():
    ...
    for pro in chats:
        try:
            await tbot.send_message(
              int(pro.chat_id), "🔰 **6:00 Am Night Mode Deactivated** 🔰 \n_Group Is Opening_. **Powered By Harley**"
            )
            await tbot(
            functions.messages.EditChatDefaultBannedRightsRequest(
